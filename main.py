...
ELECTRICAL_KEYWORDS = [
    ...
    "lighting", "high mast", "pump", "motor", "panel", "solar", "cables","cable",
    ...
    "CCTV", "cctv","CCTV camera","air conditioning","air conditioner","ac","LAN work","LAN",
    "fire alarm","fire alarm system","fire alarm installation", "fire alarm work",
    "LAN", "Switch", "TV", "AMC of PC", "Printer", "UPS", "Upgradation of PC", 
    "Annual Maintance of Computer", "Laptop", "AMC of Laptops", "Annual Maintance of Laptops",
    "Toner", "Toner refiling", "Spare Part Computer", "Printer Repairing", 
    "Construstion of cement road", "construction of concrete road", "construction of cement drain",
...
TOPIC_MAP = {
    "Maintenance": os.getenv("THREAD_MAINT"),
    "Substation": os.getenv("THREAD_SUB"),
    "Preventive": os.getenv("THREAD_PREV"),
    "Transportation": os.getenv("THREAD_TRANS"),
    "Supply": os.getenv("THREAD_SUPPLY"),
    "Others": os.getenv("THREAD_OTHERS"),
    "Mahatenders": os.getenv("THREAD_MAHATENDERS")
}

def classify_tender(description):
    desc_lower = description.lower()
    categories = {
        'Maintenance': ['maint', 'repair', 'oil', 'rectification', 'breakdown'],
        'Substation': ['substation', 's/stn', 'transformer', 'vcb', 'tc', '33kv', '11kv'],
        'Preventive': ['preventive', 'pm', 'testing', 'inspection', 'pre-monsoon'],
        'Transportation': ['vehicle', 'truck', 'transport', 'jeep'],
        'Supply': ['supply', 'delivery', 'material', 'purchase']
    }
...
async def send_telegram_message(formatted_msg, category, max_retries=3):
    if not TELEGRAM_BOT_TOKEN or TELEGRAM_BOT_TOKEN == "your_bot_token_here" or not TELEGRAM_CHAT_ID or TELEGRAM_CHAT_ID == "your_chat_id_here":
    ...
    if thread_id:
        try:
            ...
            print(f"⚠️ Warning: Invalid thread ID '{thread_id}' for category {category}")
...
def check_tenders(pending_msgs, archive):
    ...
    for item in rows:
        ...
        tech_bid_open = format_epoch(item.get("techBidOpenningDate"))
        submission_due = format_epoch(item.get("technicalBidToDate"))
        ...
        if matched_division:
            ...
            pending_msgs.setdefault(matched_division, []).append((tender_no, msg, category))
    ...
    for div_name, aliases in TARGET_DIVISIONS.items():
        ...
        while current_page <= page_limit:
            ...
            for row in rows[1:]:
                ...
                # Filter 2: Electrical Type check (Strict)
                is_electrical = False
                ...
                if is_electrical:
                    category = classify_tender(title_text)
                    msg = (
                    ...
                    pending_msgs.setdefault(div_name, []).append((ref_no, msg, category))
...
def job():
    ...
    for division, msgs in mahadiscom_msgs.items():
        ...
        for ref_no, text, category in msgs:
            success = asyncio.run(send_telegram_message(text, category))
            if success:
                archive.append(ref_no)
                new_tenders_found = True
                sent_mahadiscom = True
            
        # Delay to prevent Telegram flood limits (20 msgs/min for groups)
        time.sleep(3)
        
    # Check if there are any Mahatenders messages to send
    has_mahatender_msgs = any(msgs for msgs in mahatenders_msgs.values())
    
    # 2. Add 10-minute conditional separation delay
    if sent_mahadiscom and has_mahatender_msgs:
        print("⏱️ Inter-platform gap: Waiting 10 minutes before sending Mahatenders alerts...")
        time.sleep(600)
        
    # 3. Process Mahatenders messages grouped by Division
    for division, msgs in mahatenders_msgs.items():
        ...
        for ref_no, text, category in msgs:
            # Force route all Mahatenders to their own dedicated thread
            success = asyncio.run(send_telegram_message(text, "Mahatenders"))
            if success:
                archive.append(ref_no)
                new_tenders_found = True
            
        # Delay to prevent Telegram flood limits
        time.sleep(3)
